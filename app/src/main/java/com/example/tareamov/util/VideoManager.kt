    ...
    suspend fun saveVideo(videoData: VideoData): VideoData = withContext(Dispatchers.IO) {
        try {
            ...
            val videoId = if (videoData.id == 0L) {
                // No ID provided, let Room autogenerate
                val newId = videoDao.insertVideo(videoData)
                Log.d("VideoManager", "Inserted new video with autogenerated ID: $newId") 
                newId
            } else {
                // ID provided, use it (REPLACE strategy will handle existing IDs)
                videoDao.insertVideo(videoData)
                Log.d("VideoManager", "Inserted/updated video with provided ID: ${videoData.id}")
                videoData.id
            }
