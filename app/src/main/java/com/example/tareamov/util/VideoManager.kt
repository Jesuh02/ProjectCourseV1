...
class VideoManager(private val context: Context) {
     ...
     */
    suspend fun updateVideo(videoData: VideoData): VideoData = withContext(Dispatchers.IO) {
        try {
            Log.d("VideoManager", "Updating video: ${videoData.title} with ID: ${videoData.id}")
            videoDao.updateVideo(videoData)
            Log.d("VideoManager", "Video updated successfully")
            return@withContext videoData
        } catch (e: Exception) {
            Log.e("VideoManager", "Error updating video", e)
            return@withContext videoData
        }
    }
